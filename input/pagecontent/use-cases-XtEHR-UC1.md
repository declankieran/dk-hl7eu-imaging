...
> NOTE: Other resources would likely be required (illustrated in the data entities), however for brevity on the main resources required within the use case are included.  There are also potentially many relevant optional and alternative flows, but have been left out again for brevity.  The API lifeline is not currently being used here, but would be the focal point for request / response interactions
